  ...
  try {
    const [rows] = await pool.query(`
      SELECT internships.id, internships.start_date, internships.end_date,
        internships.mentor_id, internships.teacher_id,
      ...
      LEFT JOIN students ON students.user_id = student.id
    `);
...
exports.assignMentor = async (req, res) => {
  ...
  if (!mentor_id) return res.status(400).json({ code: "MENTOR_REQUIRED" });
  let connection;
  try {
...
exports.assignTeacher = async (req, res) => {
  ...
  if (!teacher_id) return res.status(400).json({ code: "TEACHER_REQUIRED" });
  let connection;
  try {
    connection = await pool.getConnection();
    await connection.beginTransaction();
...
exports.getInternshipById = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.execute(`
      SELECT internships.id, internships.mentor_id, internships.teacher_id,
        internships.start_date, internships.end_date,
        internship_requests.company, internship_requests.description, internship_requests.status,
        student.firstname AS student_firstname, student.lastname AS student_lastname,
        students.studyprogram,
        mentor.firstname AS mentor_firstname, mentor.lastname AS mentor_lastname,
        teacher.firstname AS teacher_firstname, teacher.lastname AS teacher_lastname
      FROM internships
      JOIN internship_requests ON internships.internship_request_id = internship_requests.id
      JOIN users AS student ON internship_requests.student_id = student.id
      LEFT JOIN students ON students.user_id = student.id
      LEFT JOIN users AS mentor ON internships.mentor_id = mentor.id
      LEFT JOIN users AS teacher ON internships.teacher_id = teacher.id
...
exports.getStudentInternships = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: "Failed to fetch student internships" });
  }
};
...
exports.getStudentInternshipById = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(`
      ...
      LEFT JOIN users AS mentor ON internships.mentor_id = mentor.id
      LEFT JOIN users AS teacher ON internships.teacher_id = teacher.id
      WHERE internships.id = ? AND internship_requests.student_id = ?

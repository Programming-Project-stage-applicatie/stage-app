...
// ─── POST /api/finale-evaluatie/internship/:internshipId/mentor-motivatie ───
router.post("/internship/:internshipId/mentor-motivatie", async (req, res) => {
  const db = req.db;
  const { internshipId } = req.params;
  const { mentor_motivatie, mentor_id } = req.body;
  if (!mentor_motivatie || mentor_motivatie.trim() === "") {
  ...
  try {
    ...
    const record = rows[0];
    if (record.status !== "submitted") {
      return res.status(400).json({ error: `Eindmotivatie kan enkel ingegeven worden als de status 'submitted' is (huidige status: '${record.status}').` });
    }
    await db.query(
      `UPDATE final_evaluations SET mentor_feedback = ?, mentor_id = ? WHERE id = ?`,
      [mentor_motivatie.trim(), mentor_id ?? null, record.id]

...
const FILTER_OPTIONS = [
  { value: 'all',       label: t('logbooks.filter.all')        },
  { value: 'submitted', label: t('logbooks.status.submitted')  },
  { value: 'approved',  label: t('logbooks.status.approved')   },
  { value: 'revision',  label: t('logbooks.status.adjustment_required') },
  { value: 'none',      label: t('logbooks.status.none')       },
];
...
export default function DocentLogboekOverzicht() {
  ...
  const stats = {
    ...
    revision:  students.filter(s => s.status === 'revision').length,
  };
    ...
    <div style={s.page}>
      ...
      {!loading && !error && (
        <div style={s.statsRow}>
          <StatCard count={stats.submitted} label={t('logbooks.status.submitted')}           bg="#fde68a" color="#78350f" />
          <StatCard count={stats.approved}  label={t('logbooks.status.approved')}            bg="#86efac" color="#14532d" />
          <StatCard count={stats.revision}  label={t('logbooks.status.adjustment_required')} bg="#fb923c" color="#fff"    />
